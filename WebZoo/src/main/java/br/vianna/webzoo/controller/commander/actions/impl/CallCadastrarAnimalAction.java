...
public class CallCadastrarAnimalAction implements ICommanderAction {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            Animal animal = new Animal(0, nome, especie, habitat, idade, dataChegada, estadoSaude, recinto, tipo, exibicao, nomeCientifico, descricao);
            ...
            request.getSession().setAttribute("mensagemSucesso", "Animal cadastrado com sucesso!");
        } catch (Exception e) {
            e.printStackTrace();
            request.getSession().setAttribute("mensagemErro", "Erro ao cadastrar animal.");
        } finally {
            if (em != null && em.isOpen()) em.close();
        }

        response.sendRedirect("web?ac=viewCadastroAnimal");
        return null;
    }
